...
import { MONTHS, QUATER_MONTHS, WEEKS } from "./datePicker.constant";
...
export const DatePicker = ({ dueDate }) => {
  ...
  const selectedMonth = MONTHS.at(date.month - 1);
  console.log(selectedMonth);

  useEffect(() => {
    setDate({
      cureentDate: new Date(dueDate),
  ...
  const handleDateSelection = (day) => {
    const date = formatDateInUTC(new Date(day.date));
    const month = getMonth(date);
    const year = getYear(date);
  ...
  return (
    <div className="bg-white shadow-2xl border py-4 w-[260px] rounded-lg flex flex-col gap-4">
      <div className="flex items-center bg-[#F3F3F3] h-[30px] p-[2px] rounded-md mx-4">
        ...
        <button
          //TODO: show premium feature modal
          className="flex flex-auto justify-center items-center text-[13px] h-fulll rounded-md"
        >
          Duration
        </button>
      </div>
      <div className="flex flex-col gap-2">
        ...
        <div
          //TODO: create timer, reminder and reminder type dropdown
      ...
      <div className="mx-4 flex justify-between gap-2">
        <button
          //TODO: clear due date of task and close date picker
          className="h-8 min-w-20 text-sm text-[rgba(25,25,25,0.6)] hover:bg-[#F4F4F4] active:bg-[#E8E8E8] border border-[rgba(25,25,25,0.1)] px-[18px] flex flex-auto items-center justify-center rounded-md transition-all"
        >
          Clear
        </button>
        <button
          //TODO: save date and close date picker
          className="h-8 min-w-20 bg-[#4772FA] text-sm text-white hover:bg-[#6C8EFB] active:bg-[#3E60CD] border border-[rgba(25,25,25,0.1)] px-[18px] flex flex-auto items-center justify-center rounded-md transition-all"
        >
          OK
        </button>
      </div>
